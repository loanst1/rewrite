...
function calculateScore() {
  ...
  // Build distance field — how far is each pixel from the nearest guide pixel?
  // Uses two-pass approximation (left-right, then right-left) for speed
  const RADIUS = 10;
  const dist = new Float32Array(w * h).fill(RADIUS + 1);
  // Mark guide pixels as distance 0
  for (let i = 0; i < charData.length; i += 4) {
    if (charData[i + 3] > 30) dist[i >> 2] = 0;
  }
  // Forward pass
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
  ...
  // Score every drawn pixel based on distance to nearest guide pixel
  // On-guide = full credit, within RADIUS = partial credit that fades with distance
  let totalCredit = 0, drawnPx = 0;
  for (let i = 0; i < w * h; i++) {
    if (drawData[i * 4 + 3] > 15) {
      drawnPx++;
      const d = dist[i];
      if (d <= 0) totalCredit += 1.0;
      else if (d <= RADIUS) totalCredit += 1.0 - (d / RADIUS) * 0.7; // fades from 1.0 to 0.3
      // else: too far from guide, no credit
    }
  }

  // Also check guide coverage — what fraction of the guide was traced nearby?
  let charPx = 0, coveredPx = 0;
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      const i = y * w + x;
      if (charData[i * 4 + 3] > 30) {
        charPx++;
        // Check if any drawn pixel within RADIUS
        let near = false;
        for (let dy = -RADIUS; dy <= RADIUS && !near; dy++) {
          for (let dx = -RADIUS; dx <= RADIUS && !near; dx++) {
            const nx = x + dx, ny = y + dy;
            if (nx >= 0 && nx < w && ny >= 0 && ny < h) {
              if (drawData[(ny * w + nx) * 4 + 3] > 15 && Math.abs(dx) + Math.abs(dy) <= RADIUS) near = true;
            }
          }
        }
        if (near) coveredPx++;
  ...
  const precision = drawnPx > 0 ? totalCredit / drawnPx : 0; // how close drawn pixels are to guide
  const recall = charPx > 0 ? coveredPx / charPx : 0; // how much of the guide was covered
  const coverage = Math.min((precision * 0.5 + recall * 0.5), 1);

  // Smoothness — rewards controlled, steady strokes (pen direction consistency)
  ...
  // Weighted: 50% coverage (precision+recall), 50% smoothness
  let score = (coverage * 0.5 + smoothness * 0.5) * 100;
  // Floor: any genuine attempt scores at least 25%
  if (hasStrokes && strokes.length > 0) score = Math.max(score, 25);
...
function showSessionSummary(avg, best, total) {
  ...
  document.getElementById('summaryScore').textContent = avg + '%';

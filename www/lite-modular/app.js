...
function loadExercise() {
  ...
  document.getElementById('exerciseType').textContent = s[typeKey] || currentType.charAt(0).toUpperCase() + currentType.slice(1);
...
function showScoreFeedback(score) {
  ...
  const bandName = score >= SCORE_BANDS.green ? (s.bandStrong || 'Strong') : score >= SCORE_BANDS.amber ? (s.bandDeveloping || 'Developing') : (s.bandEarly || 'Early stage');
...
function showSessionSummary(avg, best, total) {
  ...
  let bestBand = best >= 80 ? (s.bandStrong || 'Strong') : best >= 50 ? (s.bandDeveloping || 'Developing') : (s.bandEarly || 'Early stage');
...
async function refreshProgress() {
  ...
  document.getElementById('statAvgScore').textContent = avg >= 80 ? (str.bandStrong || 'Strong') : avg >= 50 ? (str.bandDeveloping || 'Developing') : (str.bandEarly || 'Early stage');
  document.getElementById('statBestScore').textContent = best >= 80 ? (str.bandStrong || 'Strong') : best >= 50 ? (str.bandDeveloping || 'Developing') : (str.bandEarly || 'Early stage');
...
function renderTrendChart(sessions) {
  ...
  [0, 25, 50, 75, 100].forEach(v => {
    ...
    ctx.fillText(v === 80 ? (_s.bandStrong || 'Strong') : v === 50 ? (_s.bandDeveloping || 'Developing') : '', padL - 4, y + 3);
  });
  ...
  const _cs = STRINGS[currentLang] || STRINGS.en;
  ctx.fillText(_cs.bandStrong || 'Strong', cssW - padR - 32, y80 - 4);
  // 50% line — amber
  ...
  ctx.fillText(_cs.bandDeveloping || 'Developing', cssW - padR - 54, y50 - 4);
  ctx.setLineDash([]);
